...
// float GetMaxDiff(const std::vector<std::vector<float>>& cpu,
//                  const std::vector<std::vector<float>>& gpu) {
//   float max_diff = 0.0;
//   CHECK_EQ(cpu.size(), gpu.size());
//   for (int i = 0; i < cpu.size(); ++i) {
//     const auto& cpu_i = cpu[i];
//     const auto& gpu_i = gpu[i];
//     CHECK_EQ(cpu_i.size(), gpu_i.size());
//     for (int j = 0; j < cpu_i.size(); ++j) {
//       max_diff = std::max(std::abs(cpu_i[j] - gpu_i[j]), max_diff);
//     }
//   }
//   return max_diff;
// }
...
TEST(ActNetSpeedSyncTest, SyncTest) {
  NetParam act_net_speed_param;
  QCHECK_OK(
      GetProtoParamById("Q2206", "act_net_speed_param", &act_net_speed_param));
  const std::string trt_model_path = act_net_speed_param.trt_model_path();
  const int max_batch_size = act_net_speed_param.max_batch_size();
  const auto batch_inputs = GenerateRandomBatchInput(act_net_speed_param);
  QCHECK_GT(max_batch_size, 0);

  // Gpu model.
  std::unique_ptr<prediction::PredictionQNN> gpu_net(
      new prediction::PredictionQNN(trt_model_path, act_net_speed_param));
  if (!gpu_net) {
    QLOG(ERROR) << "Failed to initialize ActNetSpeedQNN GPU.";
    return;
  }
  ...
  std::unique_ptr<prediction::PredictionQNN> cpu_net(
      new prediction::PredictionQNN(trt_model_path, act_net_speed_param));
  if (!cpu_net) {
    QLOG(ERROR) << "Failed to initialize ActNetSpeedQNN CPU.";
    return;
  }
