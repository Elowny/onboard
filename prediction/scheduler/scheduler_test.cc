...
TEST(SchedulerTest, SchedulePredictionTest) {
  ...
  *objects_proto.add_objects() = std::move(obj2);
  ...
  auto object_results =
      SchedulePrediction(context, model_pool, obj_sampler, objs_to_predict,
  ...
  ModelPool model_pool_noa(*param_manager, *param_finder, {});
  auto object_results_noa =
      SchedulePrediction(context, model_pool_noa, obj_sampler, objs_to_predict,
                         /*thread_pool=*/nullptr, &debug_proto);
  EXPECT_EQ(object_results_noa[id1].id, id1);
  EXPECT_EQ(object_results_noa[id1].trajectories.size(), kActNetJ5ModelTrajNum);
  EXPECT_EQ(object_results_noa[id2].id, id2);
  EXPECT_GE(object_results_noa[id2].trajectories.size(), 1);
}
