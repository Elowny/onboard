...
static void BM_ActnetJ5PrepareInputs(benchmark::State& state) {  // NOLINT
  NetParam act_net_j5_param;
  CHECK_OK(GetProtoParamById("Q0001", "act_net_j5_param", &act_net_j5_param));
  actnetj5::ActNetJ5Inferencer act_net_inferencer(act_net_j5_param);
  ...
  // 2. run bm
  for (auto _ : state) {
    benchmark::DoNotOptimize(act_net_inferencer.GenBatchInputs(
        objects_history, obj_sampler, &map_sampler, agent_origins));
  }
}

static void BM_ActnetJ5PredictForObjects(benchmark::State& state) {  // NOLINT
  NetParam act_net_j5_param;
  CHECK_OK(GetProtoParamById("Q0001", "act_net_j5_param", &act_net_j5_param));
  actnetj5::ActNetJ5Inferencer act_net_inferencer(act_net_j5_param);
  const int max_bs = act_net_j5_param.max_batch_size();
...
BENCHMARK(BM_ActnetJ5PrepareInputs);
BENCHMARK(BM_ActnetJ5PredictForObjects);
}  // namespace prediction
}  // namespace qcraft
