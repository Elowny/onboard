...
static void BM_LaneSelectionNetJ5PrepareInputs(
  ...
  // 2. run bm
  for (auto _ : state) {
    benchmark::DoNotOptimize(lane_selection_net_inferencer.GenBatchInputs(
        agent_dps_map, obj_sampler));
  }
}

static void BM_LaneSelectionNetJ5PredictForObjects(
    benchmark::State& state) {  // NOLINT
  NetParam lane_selection_net_j5_param;
  CHECK_OK(GetProtoParamById("Q0001", "lane_selection_net_j5_param",
                             &lane_selection_net_j5_param));
  ...
  // warm up
  lane_selection_net_inferencer.PredictForObjects(obj_sampler, agent_dps_map);
  lane_selection_net_inferencer.PredictForObjects(obj_sampler, agent_dps_map);

  // 2. run bm
  for (auto _ : state) {
    benchmark::DoNotOptimize(lane_selection_net_inferencer.PredictForObjects(
        obj_sampler, agent_dps_map));
  }
}

static void BM_LaneSelectionNetJ5ExtractOutputs(
    benchmark::State& state) {  // NOLINT
  NetParam lane_selection_net_j5_param;
  CHECK_OK(GetProtoParamById("Q0001", "lane_selection_net_j5_param",
                             &lane_selection_net_j5_param));
  ...
  const std::vector<lane_selection_net::AgentDpsIndexInfo> batch_idx_info =
      lane_selection_net_inferencer.GenBatchInputs(agent_dps_map, obj_sampler);
  // warm up
  lane_selection_net_inferencer.ExtractOutputs(batch_idx_info);
  lane_selection_net_inferencer.ExtractOutputs(batch_idx_info);
  // 2. run bm
  for (auto _ : state) {
...
BENCHMARK(BM_LaneSelectionNetJ5PredictForObjects);
BENCHMARK(BM_LaneSelectionNetJ5PrepareInputs);
BENCHMARK(BM_LaneSelectionNetJ5ExtractOutputs);
}  // namespace prediction
}  // namespace qcraft
BENCHMARK_MAIN();
