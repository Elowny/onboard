...
#include "onboard/global/trace.h"             // for Trace
...
DECLARE_uint32(trace_buffer_size_per_thread);
DECLARE_bool(truncate_trace);
...
static void RunBenchmark() {  // NOLINT
  ...
  for (int i = 0; i < max_bs; ++i) {
    const auto& hist = input.objects_history->at(std::to_string(i));
  ...
  Trace::Instance()->GenerateReport("./act_net_j5_predictor_trace.report");
}
...
int main() {
  FLAGS_trace_buffer_size_per_thread = 100000;
  FLAGS_truncate_trace = false;
  qcraft::prediction::RunBenchmark();
  return 0;
}
