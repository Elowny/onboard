...
TEST(VehicleLaneFollowPredictorTest, TwoTrajTest) {
  ...
  *objects_proto.add_objects() = std::move(obj);
  ...
  const auto trajs = MakeVehicleLaneFollowPrediction(
      obj_sampler.GetResampledMotionHistoryById(id), context, scenario,
      /*ignore_off_road=*/false);
  EXPECT_EQ(trajs.size(), 2);
}

TEST(VehicleLaneFollowPredictorTest, CtraTrajTest) {
  const auto& psmm = planner::CreateDojoTestPSMM();

  const Vec2d pos(-24.7, 49.5);
  std::string id("1");
  auto obj = planner::PerceptionObjectBuilder()
                 .set_id(id)
                 .set_type(OT_VEHICLE)
...
TEST(VehicleLaneFollowPredictorTest, OneTrajTest) {
  const auto& psmm = planner::CreateDojoTestPSMM();
