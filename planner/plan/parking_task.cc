...
DEFINE_int64(
    parking_task_global_reference_type, 1,
...
absl::Status ParkingTaskMain(const ParkingTaskInput& input,
  ...
  const auto* parking_spot_info =
      input.psmm->FindParkingSpotByIdOrNull(input.parking_spot_id);
  ...
  switch (parking_spot_info->type()) {
    ...
    case mapping::ParkingSpotInfo::Type::CUSTOM:
      return absl::NotFoundError(
          absl::StrCat("Parking spot ", input.parking_spot_id, " wrong type ",
                       parking_spot_info->type()));
  }
  state->set_task_type(task_type);

  // Set global path reference source.
  if (FLAGS_parking_task_global_reference_type == 0) {
    state->mutable_global_goal_ref()->set_source_type(
        GlobalGoalReferenceProto::NONE);
  } else if (FLAGS_parking_task_global_reference_type == 1) {
    state->mutable_global_goal_ref()->set_source_type(
        GlobalGoalReferenceProto::HD_MAP);
  } else if (FLAGS_parking_task_global_reference_type == 2) {
    state->mutable_global_goal_ref()->set_source_type(
        GlobalGoalReferenceProto::PARKING_SPOT);
  } else {
    QLOG(FATAL) << "Unexpected FLAGS_parking_task_global_reference_type: "
                << FLAGS_parking_task_global_reference_type;
  }

  result->debug_proto.mutable_freespace_task_debug()->set_parking_spot_id(
      input.parking_spot_id.value());
  ...
  if (input.reset || !HasGoal(state->global_goal_ref())) {
    ...
    if (task_type == FreespaceTaskProto::PERPENDICULAR_PARKING) {
      ...
      path_point_goal = MaybeAdjustGoal(
          ...
          input.psmm, input.object_manager, stalled_objects, path_point_goal,
          /*is_parking_task=*/true, /*adjust_dir=*/'F', kMaxAdjustDist,
      ...
      const Vec2d start_pos(input.pose->pos_smooth().x(),
                            input.pose->pos_smooth().y());
      const char adjust_dir =
          (start_pos - goal_pos).CrossProd(goal_tangent) < 0.0 ? 'L' : 'R';
      path_point_goal = MaybeAdjustGoal(
          input.freespace_params->path_finder_params(), *input.veh_geo_params,
          input.vehicle_models_params->freespace_vehicle_octagon_model_params(),
          input.psmm, input.object_manager, stalled_objects, path_point_goal,
          /*is_parking_task=*/true, adjust_dir, kMaxAdjustDist, kAdjustStep,
          kPlannerBuffer);
    }
  } else {
    path_point_goal = RestoreSmoothGoalFromGlobalRef(state->global_goal_ref(),
  ...
  switch (state->global_goal_ref().source_type()) {
    ...
    case GlobalGoalReferenceProto::PARKING_SPOT: {
      ...
      mutable_ref->set_parking_spot_id(input.parking_spot_id.value());
      const Vec2d spot_dir = parking_spot_info->unit_direction();
      const Vec2d spot_centroid = parking_spot_info->polygon().centroid();
  ...
  ASSIGN_OR_RETURN(
      ...
      ConstructFreespaceMap(
          ...
          *input.veh_geo_params, input.psmm, *input.pose, parking_spot_info,
          path_point_goal));
