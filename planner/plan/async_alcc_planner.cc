...
// x: current speed, kph; y: compensation time, second.
const PiecewiseLinearFunction kPoseDelayCompensationTimePlf(
    std::vector<double>{20, 80}, std::vector<double>{0.0, 1.2});

absl::Status UpdateLowFreqEstResult(
    bool on_vision_map, const PlannerObjectManager& object_manager,
...
Future<PlannerStatus> ScheduleFutureMultiTaskEst(
  ...
  return ScheduleFuture(
      thread_pool,
      [thread_pool, future_multi_task_est_result, &vehicle_params,
       psmm = input.planner_semantic_map_manager, pose = *input.pose,
       autonomy_state = *input.autonomy_state, alcc_params = *input.alcc_params,
       plan_start_point_info = *input.plan_start_point_info,
       plan_time = input.plan_time, st_traj_mgr = input.st_traj_mgr,
       ...
       assist_plan_state = *input.assist_plan_state,
       ...
       is_standwait = input.is_standwait]() {
        return RunMultiTasksAlccPlanner(
            MultiTasksAlccPlannerInput{
                ...
                .use_online_semantic_map = use_online_semantic_map,
...
void FakeAlccOnlyMultiTaskResult(PathBoundedEstPlannerOutput* output) {
  output->alc_state = ALC_STANDBY_ENABLE;
  output->lc_direction = LCD_NONE;

  const auto& result_list = output->est_planner_output_list;
  output->origin_lane_path =
      (!result_list.empty() &&
       !result_list.front().scheduler_output.drive_passage.empty())
          ? result_list.front().scheduler_output.drive_passage.lane_path()
          : mapping::LanePath();
  output->target_lane_path = output->origin_lane_path;
}

PlannerStatus RunDeciderAndSpeedPlanner(
    const MultiTasksAlccPlannerInput& input,
    const PlannerSemanticMapManager& low_freq_psmm,
    const AsyncMultiTaskEstOutput& latest_est_result,
    AsyncPlannerOutput* output, ThreadPool* thread_pool) {
  SCOPED_QTRACE("RunDeciderAndSpeedPlanner");

  const auto& est_output = latest_est_result.est_output;
  auto& chart_data = output->chart_data;
  ...
  const auto& alcc_params = *input.alcc_params;
  const auto& decision_config = alcc_params.decision_constraint_config();
  const auto& motion_constraint_params = alcc_params.motion_constraint_params();
  ...
  std::unique_ptr<SpacetimeTrajectoryManager> st_traj_mgr_ptr = nullptr;
  if (FLAGS_planner_alcc_use_st_traj_cutin_filter) {
    st_traj_mgr_ptr = std::make_unique<SpacetimeTrajectoryManager>(
        BuildSpacetimeTrajectoryManagerWithStTrajCutinFilter(
            sl_boundary, drive_passage, *input.object_manager.get(),
            plan_start_point, vehicle_geom_params,
            low_freq_psmm.IsOnVisionMap(), thread_pool));
  } else {
  ...
  const auto& st_traj_mgr = *st_traj_mgr_ptr.get();
  ...
  const auto& route_target_info = latest_est_result.route_target_info;
  ...
  DeciderInput decider_input{
      ...
      .scene_reasoning = nullptr,
      .enable_stop_polyline_stopping =
          ext_cmd_status.enable_stop_polyline_stopping,
      .is_engage_steer_only = input.is_engage_steer_only,
  ...
  const auto& prev_st_path_points =
      latest_est_result.st_path_points_including_past;
  const auto& st_path_points =
      FLAGS_planner_enable_cross_iteration_tf && !prev_st_path_points.empty()
          ? prev_st_path_points
          : est_output.st_path_points;

  std::vector<PathPoint> aligned_st_path_points;
  RETURN_PLANNER_STATUS_OR_ASSIGN(
      aligned_st_path_points,
      AlignPathWithPlanStartPoint(st_path_points, plan_start_point),
  ...
  const SpeedFinderInput speed_input{
      ...
      .driving_map_topo = latest_est_result.driving_map_topo.get(),
      .psmm = &low_freq_psmm,
  ...
  const auto& speed_finder_params = alcc_params.speed_finder_params();
  ...
  const bool valid = ValidateEstTrajectory(
      ...
      input.plan_start_point_info->full_stop, scheduler_output,
      vehicle_geom_params, vehicle_drive_params, motion_constraint_params,
      output->traj_points, &output->traj_validation_result, thread_pool);
  if (!valid) {
...
PlannerStatus TryRetrieveLowFreqResult(
    bool must_retrieve_result, int counter, AsyncPlannerOutput* output,
    AsyncPlannerState* async_planner_state,
    std::shared_ptr<const PlannerSemanticMapManager>* low_freq_psmm) {
  ...
  if (!async_planner_state->future_multi_task_est_status.IsValid() &&
    ...
    return PlannerStatus(
        ...
        "The latest ALCC multi-task est planner is not available yet.");
  }

  const bool is_low_freq_ready =
      async_planner_state->future_multi_task_est_status.IsReady();
  const bool is_alcc_planner_async =
      IsPlannerAsync(FLAGS_planner_alcc_async_low_freq_cycle_iterations);
  if (!IsDSimMode() && !is_low_freq_ready && must_retrieve_result &&
      is_alcc_planner_async) {
    QISSUEX(
        QIssueSeverity::QIS_ERROR, QIssueType::QIT_PERFORMANCE,
        QIssueSubType::QIST_PLANNER_PROCESS_TIMEOUT,
        absl::StrCat("Async alcc low-freq module did not finish in ",
                     FLAGS_planner_max_alcc_async_iterations, " iterations."));
  }

  if (!is_low_freq_ready && is_alcc_planner_async) {
    QLOG(WARNING) << absl::StrCat(
        "Async alcc low-freq module should be ready but is still running in ",
        counter, " iterations.");
    QEVENT_EVERY_N_SECONDS(
        "zixuan", "alcc_low_freq_timeout",
        /*seconds=*/1.0, [](QEvent* qevent) {
          qevent->AddField("expected_iter",
                           FLAGS_planner_alcc_async_low_freq_cycle_iterations);
        });
  }

  if (must_retrieve_result || !is_alcc_planner_async || is_low_freq_ready) {
    ...
    if (!est_status.ok()) {
      ...
      return PlannerStatus(
          ...
          absl::StrCat("The latest ALCC multi-task est planner failed: ",
                       est_status.message()));
    }

    auto& mutable_multi_task_est_result = *output->retrived_low_freq_output;
    if (!IsRunModeL4() || is_alcc_planner_async) {
      SCOPED_QTRACE("CreateLatestMultiTaskEstResult");

      if (async_planner_state->task_transition) {
        FakeAlccOnlyMultiTaskResult(&mutable_multi_task_est_result);
      }
      // Find psmm in low freq planner.
      if (mutable_multi_task_est_result.low_freq_psmm != nullptr) {
        *low_freq_psmm = mutable_multi_task_est_result.low_freq_psmm;
      }
      // Backward extend lane path.
      auto final_origin_lane_path_with_behind =
          mutable_multi_task_est_result.origin_lane_path.IsEmpty()
              ? mutable_multi_task_est_result.origin_lane_path
              : BackwardExtendLanePath(
                    *(low_freq_psmm->get()),
                    mutable_multi_task_est_result.origin_lane_path,
                    kDrivePassageKeepBehindLength);
      auto final_target_lane_path_with_behind =
          mutable_multi_task_est_result.target_lane_path.IsEmpty()
              ? mutable_multi_task_est_result.target_lane_path
              : BackwardExtendLanePath(
                    *(low_freq_psmm->get()),
                    mutable_multi_task_est_result.target_lane_path,
                    kDrivePassageKeepBehindLength);
      async_planner_state->latest_multi_task_est_result =
          std::make_shared<AsyncMultiTaskEstOutput>(AsyncMultiTaskEstOutput{
              ...
              .lc_direction = mutable_multi_task_est_result.lc_direction,
              .origin_lane_path = std::move(final_origin_lane_path_with_behind),
              .target_lane_path = std::move(final_target_lane_path_with_behind),
              .online_map_id = mutable_multi_task_est_result.online_map_id,
...
PlannerStatus RunAsyncAlccPlanner(
  ...
  SCOPED_QTRACE("RunAsyncAlccPlanner");
  ...
  auto low_freq_psmm = input.planner_semantic_map_manager;

  const bool should_retrieve_result =
      (!async_planner_state->task_transition &&
       ShouldRetrieveLowFreqResult(
           counter, FLAGS_planner_alcc_async_low_freq_cycle_iterations)) ||
      (async_planner_state->task_transition &&
       ShouldRetrieveLowFreqResult(
           async_planner_state->counter,
  ...
  const bool must_retrieve_result =
      (!async_planner_state->task_transition &&
       MustRetrieveLowFreqResult(counter, max_alcc_iter)) ||
      (async_planner_state->task_transition &&
       MustRetrieveLowFreqResult(async_planner_state->counter,
                                 max_cruise_iter));

  output->low_freq_result_retrived = false;
  if (should_retrieve_result) {
    auto status =
        TryRetrieveLowFreqResult(must_retrieve_result, counter, output,
                                 async_planner_state, &low_freq_psmm);
  ...
  if (ShouldRunAccPlannerInAsyncPlanner(*async_planner_state)) {
    ...
    auto acc_planner_status = RunAccPlanner(
        AccTaskInput{
            .planner_semantic_map_manager = low_freq_psmm.get(),
            .pose = input.pose,
            .steering_percentage =
            ...
            .lcc_cruising_speed_limit = ext_cmd_status.lcc_cruising_speed_limit,
  ...
  if (!should_retrieve_result) {
    // Find online map used in low freq planner.
    SCOPED_QTRACE("UpdateLowFreqSemanticMap");
    if (output->latest_low_freq_output->low_freq_psmm != nullptr) {
      low_freq_psmm = output->latest_low_freq_output->low_freq_psmm;
    }
  }
  ...
  auto origin_lp_or = ClampLanePathFromPos(
      *low_freq_psmm, latest_est_result.origin_lane_path, ego_xy);
  if (origin_lp_or.ok()) {
    output->origin_lane_path = std::move(origin_lp_or).value();
  } else {
    QLOG(INFO) << origin_lp_or.status();
  }

  auto target_lp_or = ClampLanePathFromPos(
      *low_freq_psmm, latest_est_result.target_lane_path, ego_xy);
  if (target_lp_or.ok()) {
    output->target_lane_path = std::move(target_lp_or).value();
  } else {
    QLOG(INFO) << target_lp_or.status();
  }

  if (const auto update_status = UpdateLowFreqEstResult(
          low_freq_psmm->IsOnVisionMap(), *input.object_manager,
          plan_start_point, input.vehicle_params->vehicle_geometry_params(),
          &latest_est_result, thread_pool);
      !update_status.ok()) {
  ...
  output->est_status = RunDeciderAndSpeedPlanner(
      input, *low_freq_psmm, latest_est_result, output, thread_pool);
  ...
  // Update alc state.
  if (output->est_status.ok()) {
    constexpr double kCrossBoundPreviewLength = 5.0;  // m.
    const auto& drive_passage =
        latest_est_result.est_output.scheduler_output.drive_passage;
    const double current_speed = plan_start_point.v();

    const auto& plc_result = latest_est_result.plc_result;
    const auto lc_cmd = plc_result.has_value() ? plc_result->lane_change_command
                                               : DriverAction::LC_CMD_NONE;
    std::optional<Vec2dProto> lane_change_target_point =
        input.assist_plan_state->has_lane_change_target_point()
            ? std::make_optional(
                  input.assist_plan_state->lane_change_target_point())
            : std::nullopt;
    output->lane_change_target_point = UpdateLaneChangeTargetPoint(
        latest_est_result.alc_state, lane_change_target_point, drive_passage);
    RETURN_PLANNER_STATUS_OR_ASSIGN(
        output->alc_state,
        UpdateAlcState(latest_est_result.alc_state, drive_passage,
                       output->traj_points, lc_cmd,
                       output->lane_change_target_point,
                       kPoseDelayCompensationTimePlf(Mps2Kph(current_speed)),
                       kCrossBoundPreviewLength),
