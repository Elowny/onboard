...
constexpr double kAccTargetWithMapOverlapThreshold = 0.3;

}  // namespace

AccTargetPerCorridor SelectAccTargetWithMap(const AccTargetInput& input) {
  FUNC_QTRACE();
  ...
  // 1. Use outer boundary to filter potential targets.
  ...
  // 2. Make leader/crossing leader, and cutin decisions.
  const auto leader_traj_ids =
      MakeLeaderDecisions(potential_targets, kAccTargetWithMapOverlapThreshold);
  ...
  output.source = AccCorridorSource::MAP;
  return output;
}
}  // namespace qcraft::planner
