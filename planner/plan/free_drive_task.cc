...
absl::Status FreeDriveTaskMain(const FreeDriveTaskInput& input,
                               FreespacePlannerStateProto* state,
                               FreeDriveTaskOutput* result,
                               ThreadPool* thread_pool) {
  FreespaceTaskProto::TaskType task_type = FreespaceTaskProto::FREE_DRIVING;
  state->set_task_type(task_type);
  result->debug_proto.mutable_freespace_task_debug()->set_task_type(task_type);
  // Currently freedrive task only support HD_MAP as global source.
  ...
  if (input.reset) {
    const auto mutable_ref =
        state->mutable_global_goal_ref()->mutable_hd_map_ref();
    mutable_ref->mutable_global_goal()->mutable_pos()->set_x(
        input.goal->pos.x());
    mutable_ref->mutable_global_goal()->mutable_pos()->set_y(
        input.goal->pos.y());
    mutable_ref->mutable_global_goal()->set_theta(input.goal->heading);
  }
...
absl::Status RunFreeDriveTask(const FreeDriveTaskInput& input,
                              ...
                              FreeDriveTaskOutput* result,
                              ThreadPool* thread_pool) {
  const auto status = FreeDriveTaskMain(input, state, result, thread_pool);
