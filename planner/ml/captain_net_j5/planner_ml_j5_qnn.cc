#include "onboard/planner/ml/captain_net_j5/planner_ml_j5_qnn.h"
...
PLANNERMLJ5QNN::PLANNERMLJ5QNN(const NetParam& net_param_origin) {
...
void PLANNERMLJ5QNN::PreProcess() {
#ifndef Q_CPU_ONLY
  if (config_.backend == qnn::BackendType::kTensorrt) {
    for (int i = 0; i < config_.model.input_infos.size(); ++i) {
...
void PLANNERMLJ5QNN::PostProcess() {
#ifndef Q_CPU_ONLY
  if (config_.backend == qnn::BackendType::kTensorrt) {
    for (int i = 0; i < config_.model.output_infos.size(); ++i) {
...
PLANNERMLJ5QNN::Run(int num_objs) {
  {
    SCOPED_QTRACE("PLANNERMLJ5QNN::Qantize Inputs.");
    PreProcess();
  }
  {
    SCOPED_QTRACE_ARG1("PLANNERMLJ5QNN::Infer", "device_ID", config_.device_id);
  ...
  {
    SCOPED_QTRACE("PLANNERMLJ5QNN::Deqantize Outputs.");
    PostProcess();
  }
