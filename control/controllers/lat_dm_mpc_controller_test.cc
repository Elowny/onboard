...
TEST(LatDmMpcControllerTest, StraightLaneTest) {
  ControlCommand cmd;
  ...
  LatDmMpcController lat_dm_mpc_controller(
      &controller_params.vehicle_geo_params, &controller_params.controller_conf,
      &steering_converter);
  ...
  for (int i = 0; i < 100; ++i) {
    lat_dm_mpc_controller.Reset(vehicle_state);
    QCHECK_OK(lat_dm_mpc_controller.ComputeControlCommand(
...
TEST(LatDmMpcControllerTest, FullTurnTest) {
  ControlCommand cmd;
  ControllerDebugProto debug_proto;
  ...
  const VehPose init_lon_pose(/*enable_yaw_consider_slip*/ false,
                              &controller_params.vehicle_geo_params,
                              vehicle_state);

  for (int i = 0; i < 100; ++i) {
    lat_dm_mpc_controller.Reset(vehicle_state);
    ASSERT_OK(lat_dm_mpc_controller.ComputeControlCommand(
        vehicle_state, trajectory_interface, steering_protection_result,
        init_lon_pose, lon_controller_output, &cmd, &debug_proto));
